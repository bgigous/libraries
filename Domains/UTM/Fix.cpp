...
Fix::Fix(XY loc, int ID_set, TypeGraphManager* highGraph, SectorGraphManager* lowGraph, vector<Fix>* fixes): 
	highGraph(highGraph), lowGraph(lowGraph), fixes(fixes), ID(ID_set), loc(loc),
...
std::list<std::shared_ptr<UAV> > Fix::generateTraffic(int step){
	...
	if (coin<p_gen){
		...
		UAV::UAVType type_id_set = UAV::UAVType(step%int(UAV::UAVType::NTYPES)); // EVEN TYPE NUMBER
