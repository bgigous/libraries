...
public:
	...
	UAV(int start_mem, int end_mem, UTMModes::UAVType t, TypeGraphManager* highGraph,
	...
	//! Gets the link ID from the location and desired next loction. Throws out of range if internal link.
	int curLinkID();
...
public:
	UAVDetail(easymath::XY start_loc, easymath::XY end_loc, UTMModes::UAVType t, TypeGraphManager* highGraph,
		std::map<edge, int>* linkIDs, UTMModes* params, SectorGraphManager* lowGraph);
	...
	void planDetailPath();	// may want to rename this so it has a UAV-named function
	virtual void planAbstractPath();
};