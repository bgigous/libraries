...
Fix::Fix(XY loc, int ID_set, bool deterministic, AStarManager* planners): 
	...
	p_gen(0.03), // change to 1.0 if traffic controlled elsewhere
...
std::list<std::shared_ptr<UAV> > Fix::generateTraffic(vector<Fix>* allFixes,std::vector<std::vector<XY> > *pathTraces){
	static int calls = 0;
	...
	if (coin<p_gen){
		...
		newTraffic.push_back(std::shared_ptr<UAV>(new UAV(loc,end_loc,pathTraces,type_id_set,planners)));
	}
